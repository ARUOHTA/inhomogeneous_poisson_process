\section{完全条件付き分布：点過程部分}

\subsection{\texorpdfstring{$(\boldsymbol{\beta}_{\mathrm{int}}, \boldsymbol{u}_{\mathrm{int}})$}{(β\_int, u\_int)} の完全条件付き分布}
...
&=
-\frac12 (\boldsymbol{\eta}_{\mathrm{int}} - \boldsymbol{z})^\top \Omega (\boldsymbol{\eta}_{\mathrm{int}} - \boldsymbol{z})
-\frac12 (\boldsymbol{\beta}_{\mathrm{int}} - \boldsymbol{b}_0^{\mathrm{int}})^\top (B_0^{\mathrm{int}})^{-1}(\boldsymbol{\beta}_{\mathrm{int}} - \boldsymbol{b}_0^{\mathrm{int}}) \\
&\hphantom{=}
-\frac12 \boldsymbol{u}_{\mathrm{int}}^\top Q_{\mathrm{int}} \boldsymbol{u}_{\mathrm{int}}
+ \text{const}\\[1mm]
&=
...
\subsection{\texorpdfstring{$\lambda^*$}{λ*} の完全条件付き分布}
...
\subsection{偽不在点過程 \texorpdfstring{$U$}{U} の完全条件付き分布}

$U$ の完全条件付き分布は，
\[
p(U \mid X,\lambda^*,\boldsymbol{\beta}_{\mathrm{int}},\boldsymbol{u}_{\mathrm{int}})
\propto
p(X,U \mid \lambda^*, q),
\]
であり，先の計算から
...
\begin{enumerate}
\item $N \sim \mathrm{Poisson}(\lambda^*|\mathcal{D}|)$ をサンプルする．
\item $j=1,\dots,N$ について，$(s_j,t_j) \sim \mathrm{Uniform}(\mathcal{D})$ を独立にサンプルし，$\eta_{\mathrm{int}}(s_j,t_j)$, $q(s_j,t_j)$ を計算する．
\item $u_j \sim \mathrm{Uniform}(0,1)$ をサンプリングし，$u_j < 1-q(s_j,t_j)$ のときだけ $(s_j,t_j)$ を $U$ に採用する．
\end{enumerate}

これにより，$\mathrm{IPP}(\lambda^*(1-q))$ のサンプルが得られる．

\subsection{Polya--Gamma 変数 \texorpdfstring{$\omega_i$}{ω\_i} の完全条件付き分布}
...
\quad
\eta_{\mathrm{int},i} = \boldsymbol{w}_{\mathrm{int}}(s_i,t_i)^\top \boldsymbol{\beta}_{\mathrm{int}} + u_{\mathrm{int}}(s_i,t_i),
}
\]
