...
\[
\begin{aligned}
p(X,U \mid \lambda^*, \boldsymbol{\beta}_{\mathrm{int}}, u_{\mathrm{int}})
&=
\exp\{-\lambda^*|\mathcal{D}|\}
(\lambda^*)^{n}
\prod_{i=1}^{n}
...
\[
\begin{aligned}
p(\boldsymbol{y}_i \mid N_i, \{\eta_k(s_i,t_i)\})
&\propto
\prod_{k=1}^K
\pi_k(s_i,t_i)^{y_{ik}}\\
&=
...
\]
と書く（$W_{\mathrm{int}}$ は各点 $(s_i,t_i)$ における $\boldsymbol{w}_{\mathrm{int}}(s_i,t_i)^\top$ を行に持つ設計行列）．また
\[
...
\subsubsection{マーク部分への Polya--Gamma の適用（概略）}
...
具体的には，ある $i$ と $k$ に対して，条件付きの「成功回数」 $y_{ik}$ と「試行回数」 $\tilde{N}_{ik}$ を適切に定義すると，
\[
\frac{\exp\{y_{ik}\eta_{ik}\}}{(1+\exp\{\eta_{ik}\})^{\tilde{N}_{ik}}}
\]
という形の項が現れ，先ほどと同じ恒等式を $b=\tilde{N}_{ik}$ で用いることにより，$\eta_{ik}$ に対する二次形式
\[
-\frac{1}{2}\xi_{ik}\eta_{ik}^2 + \tilde{\kappa}_{ik}\eta_{ik}
\]
として表現できる．
...
\right\}
\]
という形に書ける．ここで $\boldsymbol{\eta}_k = W_z \boldsymbol{\beta}_k + \boldsymbol{u}_k$，$\Omega_k$ は対角行列，$\boldsymbol{z}_k$ は $\xi_{ik}$ から定まるベクトルである．従って，$\boldsymbol{\beta}_k$ と $\boldsymbol{u}_k$ についても，事前がガウスである限り，完全条件付き分布はガウスとなる．
