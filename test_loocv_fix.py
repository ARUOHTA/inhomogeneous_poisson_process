...
def main():
    ...
    try:
        ...
        print(
            f"観測構成比: {evaluator.observed_compositions[test_period][test_site_id]}"
        )

        # 複数試行を実行
        print(f"\n{loocv_config.n_trials}試行のLOOCVを実行中...")
        period_result = evaluator.run_period_evaluation(
            test_period, n_trials=loocv_config.n_trials
        )
        ...
        if stats.get("n_successful_trials", 0) > 0:
            print(
                f"平均Aitchison距離: {stats.get('mean_aitchison_distance', np.nan):.4f} ± {stats.get('std_aitchison_distance', np.nan):.4f}"
            )
            print(
                f"平均Total Variation: {stats.get('mean_total_variation', np.nan):.4f} ± {stats.get('std_total_variation', np.nan):.4f}"
            )
