...
### 必読ドキュメント
**論文執筆作業を行う際は、必ず以下を確認すること：**

| ファイル | 内容 |
|---------|------|
| `260128_master_thesis/WRITING_GUIDELINES.md` | 執筆ルール・文体・引用方針 |
| `260128_master_thesis/PROGRESS.md` | 進捗管理・TODO・次のアクション |

### 概要
- 論文ディレクトリ: `260128_master_thesis/`
- 参考資料: `docs/` 内のゼミ資料・モデル説明文書
- 主要実験ノートブック: `notebooks/29_marked_point_process_distance_prior.ipynb`

### 執筆の基本原則
1. **「である調」で統一**（です・ます調禁止）
2. **断定・過剰表現を避ける**（「画期的」「非常に」等は禁止）
3. **新規の主張・数値を創作しない**（不明点は「不足している」と明示）
4. **docs/のゼミ資料を基に執筆**する
5. **PROGRESS.mdを常に最新に保つ**（作業後に更新）
