...
class CompositionalMetrics:
    ...
    def variation_array(self, compositions: np.ndarray) -> np.ndarray:
        ...
        for i in range(n_components):
            for j in range(n_components):
                if i != j:
                    log_ratios = np.log(
                        validated_compositions[:, i] / validated_compositions[:, j]
                    )
    ...
    def compute_all_metrics(self, observed: np.ndarray, predicted: np.ndarray) -> dict:
        """
        全ての評価指標を一括計算
...
def jensen_shannon_divergence(x: np.ndarray, y: np.ndarray) -> float:
    ...
    if x_sum == 0 and y_sum == 0:
        return 0.0  # 両方ゼロの場合は距離0
    elif x_sum == 0 or y_sum == 0:
        return np.log(2)  # 一方がゼロの場合は最大距離
