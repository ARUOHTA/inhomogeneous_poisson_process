...
class NadarayaWatsonEstimator(BaseCompositionModel):
    ...
    def predict_site_ratios(
        ...
        """遺跡での産地構成比予測（BaseCompositionModel準拠）"""
        # 既存のpredict_all_periods_originsを呼び出して結果を変換
        time_periods = {0: "早期・早々期", 1: "前期", 2: "中期", 3: "後期", 4: "晩期"}
        origins = ["神津島", "信州", "箱根", "高原山", "その他"]
        _, ratio_sites_df = self.predict_all_periods_origins(
        ...
        # 各時期について処理
        for period_idx in time_periods.keys():
            # この時期に関連するカラムを特定
            period_columns = ["遺跡ID"]
            for origin in ["神津島", "信州", "箱根", "高原山"]:  # "その他"を除外
                col_name = f"ratio_{period_idx}_{origin}"
                if col_name in ratio_sites_df.columns:
                    period_columns.append(col_name)

            # このperiodのデータがある場合のみ追加
            if len(period_columns) > 1:  # 遺跡ID以外のカラムがある場合
                period_df = ratio_sites_df.select(period_columns)
                result_dict[str(period_idx)] = period_df
    ...
    def predict_grid_ratios(
        ...
        """グリッド点での産地構成比予測（BaseCompositionModel準拠）"""
        time_periods = {0: "早期・早々期", 1: "前期", 2: "中期", 3: "後期", 4: "晩期"}
        origins = ["神津島", "信州", "箱根", "高原山", "その他"]
    ...
    def results(self) -> Dict[str, any]:
        """学習結果"""
        return {
            "weights_grid": self._weights,
            "weights_sites": self._weights_sites,
            "sigma": self.sigma,
