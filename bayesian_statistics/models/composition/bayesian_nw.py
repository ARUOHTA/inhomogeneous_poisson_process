...
class BayesianNadarayaWatson(NadarayaWatsonEstimator):
    ...
    def __init__(
        ...
        gamma_0 : float, default=1.0
            事前の擬似カウント総数
        """
        super().__init__(sigma, sigma_for_sites, variable_names)
    ...
    def fit(self, preprocessor: ObsidianDataPreprocessor) -> Dict[str, np.ndarray]:
        ...
        """
        # 親クラスの重み計算をそのまま実行
        weights_dict = super().fit(preprocessor)
    ...
    def predict_site_ratios(
        ...
        # 既に辞書形式なので、キーを文字列に変換して"その他"を除外
        standardized_dict = {}

        if isinstance(ratio_sites_dict, dict):
            for period_idx, period_df in ratio_sites_dict.items():
                # 主要4産地のカラムのみ選択
                select_columns = ["遺跡ID"]
                ...
                if len(select_columns) > 1:  # 遺跡ID以外のカラムがある場合
                    standardized_dict[str(period_idx)] = period_df.select(
                        select_columns
                    )
