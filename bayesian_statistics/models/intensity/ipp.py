...
class InhomogeneousPoissonProcess(BaseIntensityModel):
    ...
    # BaseIntensityModel統一インターフェース実装

    def predict_probability(self, preprocessor) -> pl.DataFrame:
        """
        グリッド上での遺跡存在確率予測（BaseIntensityModel統一インターフェース）
        ...
        # 存在確率を予測
        probabilities = self.predict_site_probability(grid_coords)

        # DataFrameとして返す
        # grid_coordsは(N, 2) [lat, lon]ラジアン形式なので度に変換
        grid_coords_deg = grid_coords / np.pi * 180

        return pl.DataFrame(
            {
                "x": grid_coords_deg[:, 1],  # lon
                "y": grid_coords_deg[:, 0],  # lat
    ...
    def predict_intensity(self, coordinates: np.ndarray) -> np.ndarray:
        ...
        return self._intensity_func.q(coordinates)
