...
def generate_IPP(
    ...
    # 応答変数 y の作成
    y = np.concatenate([np.ones(n_X), np.zeros(n_U)]).astype(int) if n_U > 0 else np.ones(n_X)
...
def plot_events_2d(
    ...
    plt.scatter(events[:, 0], events[:, 1], c='red', s=10, label=f'Observed Events (N={len(events)})', marker='x')
    plt.xlabel('X')
    plt.ylabel('Y')
    if plot_intensity:
...
def plot_intensity_posterior_2d(
    ...
    if events is not None:
        plt.scatter(events[:, 0], events[:, 1], c='red', marker="x",  s=10, label='Observed Events')
        plt.legend()
