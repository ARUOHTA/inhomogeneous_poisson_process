...
class Model3Config:
    ...
    # 説明変数（NW推定用）
    nw_variable_names: List[str] = field(default_factory=lambda: [
        "average_elevation",
        "average_slope_angle",
        "cost_kouzu",
        "cost_shinshu",
    ...
    # 説明変数（IPP用）
    ipp_variable_names: List[str] = field(default_factory=lambda: [
        "average_elevation",
        "average_slope_angle",
        "cost_shinshu",
        "cost_river",
...
class Model3Pipeline:
    ...
    def run_nadaraya_watson(
        ...
        # データフレームの更新
        preprocessor._df_elevation = preprocessor.df_elevation.join(ratio_df, on=["x", "y"])
